...
/**
 * Creates an instance of the current system state, which indicates if an
 * adaptation is necessary. A symptom is added to the hashmap if it is outside
 * the set parameters.
 * 
 * @author eric
 *
...
//	}
	...
	/*
	 * Method to indicate the current state of the system by counting the number of
	 * reported symptoms by the analysis process if the number of values that need
	 * adjustment is more than half, then the state is set to true. true state means
	 * the system needs a new adaptation plan to be formed to follow the slos
	 */
	public void setState() {
		...
		if (count >= Math.round(0.2 * current_reqs.values().size())) {
			this.state = true;
		}
	}
	 ...
	 * 
	 * @return true state meaning the system needs a new adaptation plan and false
	 *         otherwise
	 */
